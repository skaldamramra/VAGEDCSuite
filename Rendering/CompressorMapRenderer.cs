        ...
        private CompressorMapCoordinates GetCoordinates(CompressorMapType type)
        {
            ...
            switch (type)
            {
                ...
                case CompressorMapType.GT20:
                    // These use vector mapping in ZedGraph, but we provide a dummy Coord for backward compatibility if needed
                    return new CompressorMapCoordinates { MapType = type, ResourceName = string.Empty };
                default:
                    return null;
            }
        }
